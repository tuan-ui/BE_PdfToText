...
public class DocDocumentController {
	...
	public ResponseEntity<ResponseAPI> save(
		...
		try {
			...
			if (result==null)
				return ResponseEntity.status(HttpStatus.BAD_REQUEST)
						.body(new ResponseAPI(null, Constants.message.SYSTEM_ERROR, 400));
			else
	...
	public ResponseEntity<ResponseAPI> attach(@RequestParam(value = "id", required = false, defaultValue = "0") UUID id) {
		try {
			...
			return ResponseEntity.status(HttpStatus.OK).body(new ResponseAPI(lstAttach, Constants.message.SUCCESS, 200));
		} catch (Exception e) {
			return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(new ResponseAPI(null, Constants.message.SYSTEM_ERROR, 500));
		}
	}
