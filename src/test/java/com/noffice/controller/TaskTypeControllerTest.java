...
public class TaskTypeControllerTest {
    ...
    void setUp() {
        ...
        taskType.setId(testId);
        taskType.setId(UUID.randomUUID());
        taskType.setTaskTypeCode("HC");
        taskType.setTaskTypeName("Hành chính");
        taskType.setTaskTypeDescription("Hành chính Description");
        taskType.setCreateAt(LocalDate.of(2025, 1, 1).atStartOfDay());
    ...
    void search_Success() throws Exception {
        Page<TaskType> page = new PageImpl<>(
                List.of(taskType),
                PageRequest.of(0, 10),
                1L
        );

        Mockito.when(taskTypeService.getListTaskType(
                anyString(), anyString(), anyString(), anyString(),
                any(Pageable.class),
                eq(partnerId)
        )).thenReturn(page);

        mockMvc.perform(MockMvcRequestBuilders.get("/api/task-type/search")
                        .param("page", "0")
                        .param("size", "10")
                        .param("taskTypeCode", "")
                        .param("taskTypeName", "")
                ...
                .andExpect(jsonPath("$.object.content[0].taskTypeCode").value("HC"))
                .andExpect(jsonPath("$.object.content[0].taskTypeName").value("Hành chính"));
    }
    ...
    void deleteDoc_Success() throws Exception {
        Mockito.when(taskTypeService.deleteTaskType(eq(testId), any(), anyLong()))
                .thenReturn("");

        mockMvc.perform(MockMvcRequestBuilders.post("/api/task-type/delete")
    ...
    void deleteMulti_Success() throws Exception {
        ...
        Mockito.when(taskTypeService.deleteMultiTaskType(anyList(), any()))
                .thenReturn("");

        mockMvc.perform(MockMvcRequestBuilders.post("/api/task-type/deleteMulti")
    ...
    void lock_Success() throws Exception {
        Mockito.when(taskTypeService.lockUnlockTaskType(eq(testId), any(), anyLong()))
                .thenReturn("");

        mockMvc.perform(MockMvcRequestBuilders.post("/api/task-type/lock")
                        .param("id", testId.toString())
                        .param("version", "1"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.message").value("success"))
    ...
    @Test
    void addTaskType_Success() throws Exception {

        // taskTypeService.saveTaskType trả "" -> success
        Mockito.when(taskTypeService.saveTaskType(any(), any()))
                .thenReturn("");

        mockMvc.perform(
                        MockMvcRequestBuilders.post("/api/task-type/add")
                                .contentType(MediaType.APPLICATION_JSON)
    ...
    @Test
    void addTaskType_Fail() throws Exception {
    ...
    @Test
    void updateTaskType_Success() throws Exception {

        Mockito.when(taskTypeService.updateTaskType(any(), any()))
                .thenReturn("");

        mockMvc.perform(
                        MockMvcRequestBuilders.post("/api/task-type/update")
                                .contentType(MediaType.APPLICATION_JSON)
                                .content(objectMapper.writeValueAsString(taskType))
                )
                .andExpect(status().isOk())
    ...
    @Test
    void updateTaskType_Fail() throws Exception {
    ...
    @Test
    void getLogDetailTaskType_Success() throws Exception {

        Mockito.doNothing().when(taskTypeService).getLogDetailTaskType(any(), any());
    ...
    @Test
    void getAllTaskType_Success() throws Exception {

        Mockito.when(taskTypeService.getAllTaskType(
                eq(partnerId)
        )).thenReturn(List.of(taskType));
    ...
    void checkDeleteMulti_Success() throws Exception {
        ...
        Mockito.when(taskTypeService.checkDeleteMulti(anyList())).thenReturn(response);

        List<DeleteMultiDTO> ids = List.of(new DeleteMultiDTO(testId,"name","code", 1L));

        mockMvc.perform(MockMvcRequestBuilders.post("/api/task-type/checkDeleteMulti")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(ids)))
                .andExpect(status().isOk());
    }
